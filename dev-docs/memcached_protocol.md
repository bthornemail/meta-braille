...
Magic
Magic number identifying the package (See Magic Byte)
Opcode
Command code (See Command Opcodes)
Key length
Length in bytes of the text key that follows the command extras
Status
Status of the response (non-zero on error) (See Response Status)
Extras length
Length in bytes of the command extras
Data type
Reserved for future use (See Data types)
vbucket id
...
For all other expiration values, the operation will succeed by seeding the value for this key with the provided initial value to expire with the provided expiration time. The flags will be set to zero.
