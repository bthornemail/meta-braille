#!/usr/bin/env node
/**
 * Document Adapters CLI Tool
 * Test and validate document-adapters features
 */

const fs = require('fs');
const path = require('path');
const Adapter = require('../adapters/document-adapters-adapter');
...
class DocumentAdaptersTool {
  ...
  async runTests() {
    console.log('Running Document Adapters tests...\n');
  ...
  async showGolden() {
    const traces = readNdjson(path.join(this.base, 'golden/document-adapters.golden.ndjson'));
    console.log(JSON.stringify(traces, null, 2));
  }

  async showNegative() {
    const traces = readNdjson(path.join(this.base, 'negative/document-adapters.negative.ndjson'));
    console.log(JSON.stringify(traces, null, 2));
  }
  ...
  showHelp() {
    console.log(
      'Document Adapters CLI Tool\n\n' +
      'Usage:\n' +
      '  node document-adapters-tool.js <command>\n\n' +
      'Commands:\n' +
      '  test      Run all tests\n' +
...
const tool = new DocumentAdaptersTool();
