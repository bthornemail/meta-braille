#!/usr/bin/env node
/**
 * Mesh Networking CLI Tool
 * Test and validate mesh-networking features
 */

const fs = require('fs');
const path = require('path');
const Adapter = require('../adapters/mesh-networking-adapter');
...
class MeshNetworkingTool {
  ...
  async runTests() {
    console.log('Running Mesh Networking tests...\n');
  ...
  async showGolden() {
    const traces = readNdjson(path.join(this.base, 'golden/mesh-networking.golden.ndjson'));
    console.log(JSON.stringify(traces, null, 2));
  }

  async showNegative() {
    const traces = readNdjson(path.join(this.base, 'negative/mesh-networking.negative.ndjson'));
    console.log(JSON.stringify(traces, null, 2));
  }
  ...
  showHelp() {
    console.log(
      'Mesh Networking CLI Tool\n\n' +
      'Usage:\n' +
      '  node mesh-networking-tool.js <command>\n\n' +
      'Commands:\n' +
      '  test      Run all tests\n' +
...
const tool = new MeshNetworkingTool();
