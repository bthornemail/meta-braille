...
relationWorker.onmessage = (message) => {
  ...
  if (type === "relation_source_loaded") {
    sourceStatusEl.textContent = `observer/wordnet ${source.kind}: ${source.status} (${source.bytes} bytes)`;
...
async function bootstrap() {
  ...
  statusEl.textContent = `symbolic-web peer=${state.peerId} channel=${state.config.channel} dialect=${state.config.dialect}`;
...
function buildStreamScene(events) {
  ...
  return {
    ...
    orbit: orbitValue,
    glyph: braille,
    title: `Braille Orbit ${orbitValue} / ${braille}`,
    caption: `The left card shows the live 240-axis orbit clock. The right card shows the 256-axis observer projection in ${state.sceneProjection.toUpperCase()} mode.`,
...
function buildSignalScene(events) {
  ...
  return {
    ...
    title: `Hexagram ${projection.hexagram} / Braille ${latest.braille ?? "⠀"}`,
    caption: "Tap Stream mode reads the canonical Braille stream as timed pulses while hexagrams appear as the compact derived class over the same transcript chain.",
    dump: signalDumpLinesFromEvents(events, 8),
    tapFrames,
    components: resolveNarratives(latest),
    meta: [
      ...
      `path=${latest.path ?? "-"}`,
      `observer-plane=wordnet/prolog/narrative`,
      `tap_view=${state.tapView}`,
    ],
    sequence: recent.map((event, index) => {
...
function buildWordNetScene(records, selection) {
  ...
  return {
    ...
    title: `Observer ${selected.label ?? "WordNet"} / ${selected.role ?? "node"}`,
    caption: gloss,
    dump: [
...
function renderCard(card, event) {
  ...
  if (card === "projection") {
    body.textContent = mode === "local"
      ? [
          `plane=local`,
          state.relationMode === "wordnet" ? `shadow-canvas-nodes=${state.relationSummary?.nodes ?? 0}` : `shadow-canvas-nodes=${state.events.length}`,
          `graph-dom=${graphRoot.children.length}`,
          state.relationMode === "wordnet" ? `render=dom/shadow-canvas/wordnet` : `render=dom/shadow-canvas`,
        ].join("\n")
      : [
          `plane=remote`,
          `shadow-scene=${state.shadowGraph?.count ?? 0}`,
          `projection=shadow-scene/svg`,
...
bootstrap().catch((error) => {
  statusEl.textContent = `symbolic-web error: ${error.message}`;
});
